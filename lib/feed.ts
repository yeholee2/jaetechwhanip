...
export const NEWS_SOURCES = [
  ...
  { name: '머니투데이', url: 'https://rss.mt.co.kr/mt_stock.xml' },
...
export function classifyNewsCategory(title = '', summary = '') {
  ...
  return matched?.category || '재테크 입문';
}
...
export function toFeedItems(newsItems: NewsItem[] = sampleNewsItems): FeedItem[] {
  return [
    ...hanipArticles.map(article => ({ ...article, type: 'column' as const })),
...
export async function fetchNewsItems(): Promise<NewsItem[]> {
  ...
  try {
    const res = await fetch(
      `${supabaseUrl}/rest/v1/news_items?select=id,source,title,summary,url,thumbnail_url,category,published_at&order=published_at.desc&limit=40`,
      {
