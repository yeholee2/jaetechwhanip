 ...
 */
import type { HomeRollingBanner, SiteBanner } from './site-settings';
import { DEFAULT_HOME_ROLLING_BANNERS } from './site-settings';
...
export async function fetchSiteSettings(): Promise<{
  ...
  try {
    ...
    return {
      ...
      rollingBanners: (map.get('home_rolling_banners') as HomeRollingBanner[]) || DEFAULT_HOME_ROLLING_BANNERS,
    };
  } catch {
    return {
      keywords: DEFAULT_KEYWORDS,
