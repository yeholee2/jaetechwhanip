...
### 변경사항
- [Rollback] `/temp-etf-v2`의 Lazyweb 반영 시안을 취소하고 이전 3안 목업 상태로 되돌림
- [Scope] 정식 `/etf` 변경 없음
- [Memo] 금융앱 요소를 그대로 섞는 방식은 재테크한입 톤과 맞지 않아 폐기

### 확인
- [x] 문제 커밋 `41f503c` revert
...
### 변경사항
- [ETF] 모바일 상단의 큰 검색 박스/버튼을 압축해 `ETF 바로 찾기` 보드처럼 보이도록 정리
- [ETF] 데이터 기준, 오늘의 ETF, 관심/최근/분배금 탭, ETF/테마/결과 수를 한 영역 안에 묶어 상단 박스가 줄줄이 쌓이는 느낌 완화
- [ETF] 랭킹 제목과 안내 문구를 더 짧고 명확하게 바꾸고, 필터표까지 더 빨리 내려갈 수 있게 화면 밀도 조정
- [ETF] 모바일 탭 텍스트가 잘리지 않도록 3분할 정렬로 수정
...
## [2026-05-10] ETF 화면 FunETF 레퍼런스 기준 재정리 — Codex
**작업자:** Codex
**태그:** #ETF #FunETFReference #UIDesign

### 변경사항
- [ETF] FunETF의 핵심 구조(통합검색, 관심/최근 탭, 많이 보는 상품 랭킹, 상품 검색 필터표)를 `/etf`에 맞춰 재구성
- [ETF] `?q=` 검색과 `?theme=` 필터를 실제로 동작하게 연결해 UI와 결과 목록이 함께 바뀌도록 수정
- [ETF] 랭킹 영역은 가격·등락률·미니 차트·순위 리스트로 구성하고, 필터 영역은 상품명/분류/총보수/분배/현재가/등락률/순자산 표 중심으로 변경
- [ETF] 모바일에서는 표가 카드형 비교 리스트로 접히게 만들어 홈 UI 톤을 유지하면서도 ETF 정보 밀도를 살림

### 확인
- [x] FunETF 레퍼런스 화면 확인
- [x] `npm run build`
- [x] 로컬 `/etf`, `/etf?q=엔비디아` 모바일 화면 확인

---

## [2026-05-10] ETF 화면 홈 UI 톤 재정리 — Codex
**작업자:** Codex
**태그:** #ETF #HomeUI #UIDesign

### 변경사항
- [ETF] `/etf`의 큰 도구형 랜딩/검은 탭/두꺼운 검색창을 제거하고 홈과 같은 탭·칩·흰 카드 리스트 구조로 변경
- [ETF] 페이지 폭을 홈 기준 1040px로 맞추고, 본문 + 280px 우측 보조영역 구조 적용
- [ETF] ETF 정보값은 유지하되 표를 카드형 리스트로 낮춰 모바일에서도 홈 피드처럼 스캔되게 정리
- [ETF] 관련 질문/스파링/한입 기준을 우측 보조 카드로 이동해 홈의 보조영역 흐름과 맞춤

### 확인
- [x] `npm run build`
- [x] 로컬 `/etf` 모바일 폭 화면 확인
...
### 변경사항
- [Sparring] 스파링 페이지 폭/상단 여백/제목 크기를 홈 화면 기준으로 축소해 별도 랜딩처럼 튀는 느낌 완화
- [Sparring] 진행중 스파링 카드를 홈 우측 스파링 카드와 같은 12px 반경·낮은 정보 밀도·파랑 중심 톤으로 정리
- [Sparring] 지난 스파링 카드를 홈 질문 카드처럼 흰 배경/얇은 라인/작은 메타 정보 중심으로 변경
- [Sparring] 카테고리 칩과 정렬 버튼을 홈의 필터 칩 스타일에 맞게 조정
- [Data] 제목이 `스파링`/`테스트`뿐이고 0표·0댓글인 임시 진행중 항목은 서비스 목록에서 숨김

### 확인
- [x] `npm run build`
- [x] 로컬 `/sparring` 데스크톱 UI 확인

---

## [2026-05-10] 홈 스파링 고정·내정보 아이콘 상시 노출 — Codex
**작업자:** Codex
**태그:** #Home #Sparring #Profile #UI

### 변경사항
- [Home] 데스크톱 홈 우측 스파링 카드가 스크롤 중에도 따라오도록 sticky 처리
- [Profile] 홈 상단 알림 옆 내정보 아이콘을 로그인 확인 중에도 항상 노출되게 수정
- [AppShell] 공통 헤더도 검색 → 알림 → 내정보 → 질문 순서로 맞추고, 비로그인 상태의 내정보 진입을 항상 표시

### 확인
- [x] `npm run build`
- [x] 로컬 홈에서 스파링 sticky 및 내정보 아이콘 노출 확인

---

## [2026-05-10] 홈 보조영역 스파링 단일화 — Codex
**작업자:** Codex
**태그:** #Home #Sparring #UI

### 변경사항
- [Home] 우측 보조영역에서 전문가 신청/ETF 정보/피드/키워드 위젯을 제거하고 현재 스파링 카드만 남김
- [Sparring] 홈과 질문 상세의 스파링 카드가 하드코딩 문구 대신 실제 진행중 스파링의 제목/투표 수/마감 시간/상세 링크를 사용하도록 연결
- [Component] 공용 `SparringMiniCard`를 추가해 여러 화면의 작은 스파링 진입 카드를 동일 데이터로 렌더링

### 확인
- [x] `npm run build`
- [x] 로컬 홈/질문 상세에서 스파링 외 보조 위젯 제거 및 현재 스파링 링크 확인

---

## [2026-05-10] 스파링 a-ha 구조 기준 단순화 — Codex
**작업자:** Codex
**태그:** #Sparring #UI #AhaReference

### 변경사항
- [Sparring] a-ha `/discussions` 실화면을 브라우저로 확인하고, 스파링 페이지 구조를 제목 → 진행중 카드 → 지난 스파링으로 단순화
- [Sparring] 공용 슬로건/상단 통계칩/진행중 설명 블록 제거로 첫 화면 요소 수 축소
- [Sparring] 모바일 진행중 카드를 a-ha처럼 가로 스크롤 카드 흐름으로 변경
- [Sparring] 지난 스파링 필터/정렬 위치를 a-ha 구조에 맞춰 제목 아래 카테고리 칩 → 기본순 순서로 정리
- [Sparring] 모바일 지난 카드에서 보조 승리 문구를 숨기고 투표/토론 메타를 더 작게 압축

### 확인
- [x] `npm run build`
- [x] 로컬 `/sparring` 모바일 화면에서 슬로건/통계칩/진행중 설명 미노출 확인
...
### 변경사항
- [Sparring] 지난 스파링 카드를 a-ha 레퍼런스처럼 결과 요약형 카드로 재구성
- [Sparring] 종료 라운드에서 우세 선택지, 퍼센트, 투표 수, 토론 수, 라운드 번호를 한 화면에 노출
- [Data] DB 종료 스파링이 적을 때 테스트용 지난 스파링 8개까지 보강 노출
- [Category] 사용자 노출명 `재테크입문`을 `재테크`로 변경, 기존 DB 값과 `/topics/재테크-입문`은 호환 유지

### 확인
- [x] `npm run build`
- [x] 로컬 `/sparring` 모바일 화면 확인
...
### 변경사항
- [스파링] 진행중 카드에서 `A vs B` 선택지 티저까지 제거하고 아하 레퍼런스처럼 이미지/투표 수/타이틀/남은 시간/참여하기만 노출
- [스파링] 데스크톱 진행중 그리드를 3열 기준으로 조정해 아하 캡처의 상단 카드 흐름에 맞춤
- [의도] 목록 카드는 투표 UI가 아니라 상세 진입을 유도하는 썸네일 카드로 유지

### 확인
- [x] `npm run build`
- [x] 로컬 `/sparring` 진행중 카드에서 `VS`/퍼센트 미노출 확인

---

## [2026-05-10] 진행중 스파링 카드 호기심 보강 — Codex
**작업자:** Codex
**태그:** #Sparring #UI #Engagement

### 변경사항
- [스파링] 진행중 카드에서 투표 퍼센트/결과 바 미리보기를 제거
- [스파링] 아하 레퍼런스처럼 `선택지 A vs 선택지 B`, 투표 수, 남은 시간, 참여하기 중심으로 노출
- [의도] 목록에서 결과를 먼저 알아버리는 문제를 줄이고 상세 진입/투표 전환을 유도

### 확인
- [x] `npm run build`
- [x] 로컬 `/sparring` 진행중 카드에서 퍼센트 미노출 확인
